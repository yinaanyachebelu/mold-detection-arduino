...
/* Includes ---------------------------------------------------------------- */
#include <ucesaua-project-1_inferencing.h>
...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        if (ei_sleep(4000) != EI_IMPULSE_OK) {
            break;
        }
        digitalWrite(LEDR, HIGH);
        digitalWrite(LEDG, HIGH);
        ...
        if (result.classification[0].value > 0.8 && result.classification[1].value < 0.2){
          ei_printf("Mold Present");
          digitalWrite(LEDB, HIGH);
          digitalWrite(LEDR, LOW); 
          digitalWrite(LEDG, HIGH);
        }
        else if (result.classification[0].value > 0.9) {
          ei_printf("Mold Present");
          digitalWrite(LEDB, HIGH);
          digitalWrite(LEDR, LOW); 
          digitalWrite(LEDG, HIGH);
